...
def graph2swc(G:NeuroGraph, mode:str='normal'):
    # convert one neuron graph (with soma) to swc
    assert mode in ['normal', 'forced'], 'Invalid mode in graph2swc()'
    skip_error = False
    if mode == 'normal':
        skip_error = False
    elif mode == 'forced':
    ...
    if len(kids2parent) != graph.number_of_nodes():
        append_log_info = 'Detected cycles or disconnected nodes in the graph.'
        logger = f'{logger} {append_log_info}'
        if flag == 'success':
            flag = 'warning'
